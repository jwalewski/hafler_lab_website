...
def export_dataframe(input_dataframe: pandas.DataFrame, style:str="Freezer_Works_Import", output_filepath:str="../data/exports/", output_filename:str=None, row_offset: Union[None,int]=None, column_offset: Union[None,int]=None) -> None:
    ...
    if style == "Freezer_Works_Import":
        ...
        columns = ["(Neuro) Patient ID", "(HaflerLab) Substudy Visit", 
        ...
        patient_report_df.to_csv(output_path)

        print("Patient report saved to:", output_path)
    else:
...
def plot_samples_by_visit(df: pandas.DataFrame, visit_col: str, patient_col: str, title: str = None):
    # Clean up the visit column — extra spaces are the enemy
    df[visit_col] = df[visit_col].str.strip()
